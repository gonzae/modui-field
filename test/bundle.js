...
},{"backbone":5,"backbone-subviews":2,"backbone-view-options":3,"jquery":8,"jquery-field-views":7,"underscore":9}],2:[function(require,module,exports){
/*
...
( function( root, factory ) {
	...
	function _getFieldViewsWithinElements( els, options ) {
	...
	$.fn.setFieldViews = function( suppliedValues, options ) {
		if( suppliedValues === undefined ) throw "Undefined hash of values to use for setFields.";
		...
		var $this = $( this );
		var fieldViews = _getFieldViewsWithinElements( $this, options );

		for( var i = 0, len = fieldViews.length; i < len; i++ ) {
			var thisFieldViewObject = fieldViews[ i ];
			var thisFieldViewIdent = thisFieldViewObject.name;
			var newValue = suppliedValues[ thisFieldViewIdent ];

			if( newValue === undefined &&
				options.resetUnsuppliedValuesToDefaults )
			{
				thisFieldViewObject.resetValueToDefault();
			}

			if( newValue !== undefined )
				thisFieldViewObject.setValue( newValue );
		}

		return this;
	};

	$.fn.getFieldViews = function( options ) {
		options = $.extend( {
			includeHiddenViews : false,
			excludeUnavailableDependents : true
		}, options );
		...
		for( var i = 0, len = fieldViews.length; i < len; i++ ) {
			var thisFieldViewObject = fieldViews[ i ];
			var thisFieldViewIdent = thisFieldViewObject.name;
			valueHash[ thisFieldViewIdent ] = thisFieldViewObject.getValue();
		}
	...
	var actions = {
		get : function() {
			return $.fn.getFieldViews.apply( this, arguments );
		},
		set : function() {
			return $.fn.setFieldViews.apply( this, arguments );
		},
		find : function() {
			return _getFieldViewsWithinElements( this, arguments );
		},
